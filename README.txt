...
4.    In the loop, the User process will request specific page numbers, which are then sent to the Scheduler.
5.    The Scheduler retrieves the content of the requested page (up to 512 bytes) using the page table and sends it to the User process.
6.    Both the Scheduler and the User process display the requested page's content to the standard output.
